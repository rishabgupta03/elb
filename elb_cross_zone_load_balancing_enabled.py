...
CONTROL_NAME = "ELB Cross-Zone Load Balancing Enabled"
...
def classify_error(e):
    ...
    Kept as one small function so error handling stays readable
    instead of a long if/else chain scattered through the script.
    """
                ...
                for lb in page.get("LoadBalancerDescriptions", []):
                    ...
                    try:
                        ...
                        if cross_zone_enabled:
                            status = "COMPLIANT"
                            compliant += 1
                            evidence = "Cross-zone load balancing is enabled"
                        else:
                            status = "NON_COMPLIANT"
                            non_compliant += 1
                            evidence = "Cross-zone load balancing is disabled"
...
def write_csv(results, account_id):
    filename = f"elb_cross_zone_load_balancing_enabled_{account_id}.csv"
...
def main():
    parser = argparse.ArgumentParser(
        description="Check whether Classic Load Balancers have cross-zone load balancing enabled."
    )
