...
CONTROL_NAME = "ELB Connection Draining Enabled"
            ...
            for page in paginator.paginate():
                for lb in page.get("LoadBalancerDescriptions", []):
                    ...
                    try:
                        ...
                        draining = attrs.get("ConnectionDraining", {})
                        draining_enabled = draining.get("Enabled", False)
                        draining_timeout = draining.get("Timeout", "N/A")

                        if draining_enabled:
                            status = "COMPLIANT"
                            compliant += 1
                            evidence = f"Connection draining is enabled (timeout: {draining_timeout}s)"
                        else:
                            status = "NON_COMPLIANT"
                            non_compliant += 1
                            evidence = "Connection draining is disabled"
...
def write_csv(results, account_id):
    filename = f"elb_connection_draining_enabled_{account_id}.csv"
...
def main():
    parser = argparse.ArgumentParser(
        description="Check whether Classic Load Balancers have connection draining enabled."
    )
