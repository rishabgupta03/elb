...
CONTROL_NAME = "Elastic Load Balancer Has Access Logs to S3 Configured"
            ...
            for page in paginator.paginate():
                for lb in page.get("LoadBalancerDescriptions", []):
                    ...
                    try:
                        ...
                        s3_bucket = access_log.get("S3BucketName", "N/A")

                        if logging_enabled:
                            status = "COMPLIANT"
                            compliant += 1
                            evidence = f"Access logging enabled to S3 bucket '{s3_bucket}'"
                        else:
                            status = "NON_COMPLIANT"
                            non_compliant += 1
                            evidence = "Access logging to S3 is not enabled"
...
def write_csv(results, account_id):
    filename = f"elb_logging_enabled_{account_id}.csv"
...
def main():
    parser = argparse.ArgumentParser(
        description="Check whether Classic Load Balancers have S3 access logging enabled."
    )
