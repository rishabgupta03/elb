...
CONTROL_NAME = "Elastic Load Balancer Is Not Internet-Facing"
            ...
            for page in paginator.paginate():
                for lb in page.get("LoadBalancerDescriptions", []):
                    ...
                    if scheme == "internet-facing":
                        ...
                        non_compliant += 1
                        evidence = "Load balancer scheme is 'internet-facing'"
                    elif scheme == "internal":
                        status = "COMPLIANT"
                        compliant += 1
                        evidence = "Load balancer scheme is 'internal'"
                    else:
                        status = "SKIPPED"
                        skipped += 1
                        evidence = f"Could not determine load balancer scheme (got '{scheme}')"
...
def write_csv(results, account_id):
    filename = f"elb_internet_facing_{account_id}.csv"
...
def main():
    parser = argparse.ArgumentParser(
        description="Check whether Classic Load Balancers are internet-facing."
    )
